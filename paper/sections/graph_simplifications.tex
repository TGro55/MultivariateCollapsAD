...
Let us recall the vectorized-then-summed 2-jet of \cref{eq:taylor-mode-scalar} for this example and assume that the Taylor coefficients are given by $\{\vx_{0,d} = \vx_0\}$ (\ie same primal value), $\{\vx_{1,d}\}$, and $\{\vx_{2,d}\}$ where $d$ indexes the directions along which we evaluate then sum:
\begin{align*}
  \begin{matrix}
    \vx_0
    \\
...
The computational graph for this procedure is displayed in the following diagram, with input and output nodes highlighted dark and light gray, and the convention that a suffix \texttt{\_d} means that a tensor collects the result of all the jets along its leading axis.
