...
For simplicity, we consider a vector-to-scalar function $g: \sR^{D} \to \sR, \vx \mapsto g(\vx)$ (the general vector-to-vector case is straightforward but requires more notation) whose Laplacian is
\begin{align*}
  \Delta g(\vx) =
  \Tr( \nabla^2 g(\vx) )
  =
  ...
  \sum_{d=1}^{D} \ve_d^{\top} \nabla^2g(\vx) \ve_d\,,
\end{align*}
with $\nabla^2 g(\vx) \in \sR^{D \times D}$ the Hessian of $f$ evaluated at $\vx$.
...
\begin{align*}
  ...
  \approx&
           ...
           =&
              \frac{1}{S} \sum_{s=1}^{S} \sum_{i,j=1}^D [\partial^2g(\vx)]_{i,j} [\vv_s]_i [\vv_s]_j
              =
              \frac{1}{S} \sum_{s=1}^{S} \left\langle \partial^2g(\vx), \vv_s^{\otimes 2} \right\rangle\,.
              ...
              =& \frac{1}{S} \left\langle \partial^2g(\vx), \sum_{s=1}^{S} \vv_s^{\otimes 2} \right\rangle\,.
\end{align*}

\subsection{Connection to the Forward Laplacian}
We start by writing out the propagation rules of the forward Laplacian (Equations (5-7) in \citet{li2023forward}) for a function $f = g \circ \vh$ with $g: \sR^C \to \sR$ whose input we denote by $\vh_0 \in \sR^C$.
