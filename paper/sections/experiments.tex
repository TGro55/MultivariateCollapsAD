...
We decided so because PyTorch's \texttt{fx} library provides a user-friendly official interface to capture and transform computational graphs of functions to apply our proposed collapsing.
...
The main advantage seems to be that we can clearly disentangle building the graph from simplifying it using linearity, whereas in JAX we could write a custom interpreter that would have to re-implement the logic of summing the $K$-th component for abitrary $K$.

\paragraph{Usage.}
Our implementation takes a PyTorch function (\eg a neural net) and first captures its computational graph using \texttt{torch.fx}'s symbolic tracing mechanism, then replaces each operation with its Taylor arithmetic.
This yields the computational graph of the function's $K$-jet.
...
And (iii), to keep the number of supported operations and graph simplifications minimal, we currently do not use in-place operations
\textcolor{red}{(I think nested first-order probably does that). \AW{AW: comment still in main text}}
...
To implement the (weighted) Laplacian and its stochastic counterpart, we use vector-Hessian-vector products (VHVPs) in forward-over-reverse order, as recommended already in \AW{Griewank \& Walther \cite{griewank2008evaluating}.}  %  by \citet{dagreou2024how}. 
\todo{I would actually prefer the reference to the AD book, since this is already explained there, but also OK if you prefer the ML reference.}
For the Biharmonic operator, we simply nest two VHVPs.
For all implementations, we measure (i) runtime, and peak memory when computing the (ii) differentiable operator (\eg required for PINNs), and (iii) the non-differentiable operator (\eg required for VMC).
The runtime measurements commence with 10 warm-up runs, followed by 50 repetitions, and we report the smallest runtime as best approximation to runtime.
...
\todo{Felix: Write down that the function is $f_\vtheta(\vx)$ and we want to compute differential operators \wrt $\vx$, then differentiate \wrt $\vtheta$ using standard backprop.}
...
\item \textbf{Collapsed Taylor mode accelerates standard Taylor mode.}
  When comparing collapsed with standard Taylor mode, we observe that the measure runtime differences correspond well with the theoretical prediction.
...
\item \textbf{Collapsed Taylor mode consistently outperforms nested 1\textsuperscript{st}-order AD.}
  ...
  For the randomized Bi-harmonic, we observe even greater runtime and memory improvements by roughly 9 and 3 (or 6 if non-differentiable)x.
