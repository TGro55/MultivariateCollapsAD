...
Our JAX experiments with the Laplacian confirm that \texttt{jit} does not affect the relative performance (\cref{sec:jax-benchmark}). 
Last, while our Taylor mode implementation is competitive with JAX's, we did not fully optimize it (\eg we do \emph{not} use in-place operations, and we do \emph{not} implement the efficient schemes from \citet[][\S13]{griewank2008evaluating}, but stick to Fa\`a di Bruno (\cref{eq:faa-di-bruno})).
...
For the weighted Laplacian's coefficient matrix, we choose a full-rank diagonal matrix.
...
\item \textbf{Collapsed Taylor mode accelerates standard Taylor mode.}
  ...
  Empirically, we measure that adding one datum adds {\color{tab-orange}$0.60$\,ms} to standard, and {\color{tab-green}$0.33$\,ms} to collapsed, Taylor mode; the ratio of $\approx 0.55$ is close.
...
\item \textbf{Collapsed Taylor mode outperforms nested 1\textsuperscript{st}-order AD.}
  ...
  in the stochastic case up to 9x in time, and 3x in memory (differentiable).
\end{itemize}
...
\Eg, when computing the exact Laplacian with nested first-order AD, PyTorch consumes $0.61$\,ms per datum (\cref{tab:benchmark}), while JAX uses $0.57$\,ms (\cref{fig:vanilla-taylor-not-enough,tab:jax-benchmark}).
...
We conclude from these results that (both ours, as well as the existing JAX) Taylor mode still has potential for improvements that may further increase the margin to nested first-order.
