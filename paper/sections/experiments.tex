...
In our PyTorch experiments, we therefore report uncompiled implementations. 
...
Last, while our Taylor mode implementation is competitive with JAX's, we did not fully optimize it (\eg we do \emph{not} use in-place operations, and we do \emph{not} implement the efficient schemes from \citet[][\S13]{griewank_evaluating_1999}, but stick to \cref{eq:faa-di-bruno}).
...
Users can compute a differential operator with this vanilla Taylor mode. 
Collapsing is achieved using the function \texttt{simplify}, which traces the computation again, rewrites the graph, and propagates the summation of highest coefficients up to the leafs.
...
For the weighted Laplacian's coefficient matrix, we choose a diagonal matrix.
As common for PINNs \cite[\eg][]{shi2024stochastic,dangel2024kroneckerfactored}, we use a 5-layer MLP $\smash{f_\vtheta}: D \to 768 \to 768 \to 512 \to 512 \to 1$ with $\tanh$ activations and trainable parameters $\vtheta$, and compute the PDE operators on multiple batch of sizes $N$.
...
\item \textbf{Collapsed Taylor mode outperforms nested 1\textsuperscript{st}-order AD.}
  ...
  For the biharmonic operator, we also observe speed-ups\footnote{For the exact biharmonic, first-order AD leverages the operator’s structure by computing it as the Laplacian of the Laplacian—unlike our Taylor mode implementations. We chose the biharmonic as a case study for a general linear operator to show that even here, collapsing Taylor yields improvements, despite increased memory use. In \cref{sec:jax-benchmark}, we confirm that nesting Laplacians—also possible in Taylor mode—offers the most efficient approach, achieving roughly 3x savings in both runtime and memory with our collapsing technique (\cref{tab:jax-benchmark,fig:jax-benchmark}).},
...
T: Kann für mich raus. Wir brauchen ja nicht für vanilla Taylor mode argumentieren}
...
We find that the choice of ML framework does not affect the results.
