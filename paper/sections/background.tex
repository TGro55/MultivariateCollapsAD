...
\paragraph{[Needs double-checking] Biharmonic operator = Laplacian(Laplacian).}
A very intuitive way to understand that the collapsing of terms can also be done for the biharmonic operator, is that it corresponds to applying the Laplacian twice.
Hence, we can collapse coefficients in the inner Laplacian, as well as the outer Laplacian.
Suppose we start with a collapsed interior Laplacian $\Delta f: \vx \in \sR^D \mapsto \Delta f(x) \in \sR$ where function $f$ propagates forward one vector.
We know from previous explanations that this requires propagating $2 + D$ vectors.
Now, let's apply the Laplacian a second time, i.e. $\Delta \tilde{f}$ where evaluating $\tilde{f} = \Delta f$ propagates $2 + D$ vectors. 
Naively we get $(2 + D) + D * (2 + D) + D * (2 + D)$, but the second-order components can again be collapsed into a single vector, so we obtain $(2 + D) + D * (2 + D) + 1 = D^2 + 3D + 3$ vectors that need to be pushed forward for the biharmonic.
\textbf{We should expect Tim's $D^2 + 2D + 3$ from below though, so there is still a slight mismatch we need to understand.}
