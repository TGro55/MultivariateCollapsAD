...
To illustrate Taylor-mode\todo{Taylor mode}, consider the scalar function $f: \sR \to \sR$ and extend the input variable $x$ to a path $x(t)$ whose form is a univariate Taylor polynomial of degree $K$, $\smash{x(t) = \sum_{k=0}^K \frac{t^k}{k!} x_k}$ with $x_k$ the $k$-th Taylor coefficient. \todo{T: das passt nicht mit unsere Jet erklärung}
...
contracting the $k$-th derivative tensor $\partial^k \vf(\vx_0) \in \sR^C \times (\sR^D)^{\otimes k}$\todo{T: Notation check please -> F: Looks good to me!?}, such that $\langle \tA, \partial^k \vf(\vx_0) \rangle \in \sR^C$. 

\paragraph{Multivariate case \& composition.}\todo{Discussion about Jet definition as it is:
T: The Jet definition allows us to naturally use Faa di Bruno because this is exactly the d/dt derivative; Otherwise we would have the problem that we have to introduce an auxiliary $t$ anyway. Therefore, we use Jets definition as it is to be able to easily explain how to compute the coefficients but we never really compute a Jet. We are interested in the last coefficients. Right? Shall we mention this somewhere? If you agree, feel free to add something}
...
\end{align*}
and requires the $K$-jet of $\vx$, $(J^K \vx)(t) := \sum_{k=0}^K \frac{t^k}{k!} \vx_k$ (illustrated in \cref{fig:utp}). $J^K \vf$ is sometimes also considered differently as a mapping $(\vx_0, \dots, \vx_K) \mapsto (\vf_0, \dots, \vf_K)$ \cite{bradbury2018jax}, because we are interested in the coefficients, not the Taylor polynomial itself. \todo{T: @Felix ok?}
...
In the simplest case, let $\vf$ be given by $\vf = \vg \circ \vh: \sR^D \to \sR^I \to \sR^C$ for two elemental functions $\vg$ and $\vh$. Given the input $K$-jet for $\vx$, the coefficients $\vh_k = \left. \frac{\mathrm{d}^k}{\mathrm{d}t^k}\vh(\vx(t)) \right|_{t=0}$ are computed using the generalized Faà di Bruno formula
\begin{align}
