...
\begin{center}
  \begin{tikzpicture}
    ...
    \draw [-Latex, \colorTM] (bottomleft.east) to node [midway, above, tab-orange] {\textcolor{\colorTM}{Taylor-mode AD}
    } (bottomright.west);
  \end{tikzpicture}
\end{center}
...
that may include also mixed-partial derivatives. Being part of a higher-order derivative tensor, they are not directly computable using a single $K$-jet. Hence, we leverage the work of Griewank et al. \cite{griewank_evaluating_1999}. They derived a technique to compute the whole $K$-th derivative tensor with a minimal number of $K$-jets propagated through the computational graph by choosing the coefficients of the $K$-jets appropriately. 
