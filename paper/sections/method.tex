...
\begin{align}\label{eq:faa-di-bruno-expanded}
  \textstyle % Comment out this line if we have enough space
  \sum_r\vf_{K, r}
  =
  \sum_r\vg_{K,r}
...
\begin{align}
  ...
  \end{cases}
\end{align}
We can directly address the stochastic version: simply draw $S$ standard normal vectors $\vv_1, \dots, \vv_S$ and propagate the coefficients $\{(\vx_{0,s} = \vx_0, \vx_{1,s} = \vv_s, \vx_{2,s} = \vx_{3,s} = \vx_{4,s} = \vzero)\}_{s=1}^S$.
With standard Taylor mode, this uses ${\color{tab-orange} 1 + S + S + S + S}$ vectors; collapsed Taylor mode uses ${\color{tab-green} 1 + S + S + S + 1}$ vectors.
...
\begin{equation}\label{eq:ttc-general}
  ...
  \right>}\,.
\end{equation}
We can use standard Taylor mode to compute each \textcolor{tab-orange}{summand}, propagating different $K$-jets with coefficients $(\vx_0, \vx_1 = \sum_i \vv_{d_i}[\vj]_i, \vx_2 = \dots =\vx_K = \vzero)$, then \textcolor{tab-green}{collapse} the sums from the tensor basis expansion.
...
Further, we can often exploit symmetries in the $\gamma_{\vi, \vj}$s and the basis vectors to further reduce the number of $K$-jets.
Please see \cref{sec:appendix_ttc} for a fully-contained example.

\begin{wrapfigure}[19]{r}{0.41\textwidth}
  \centering
  \vspace{-1ex}
  \input{figures/ttc_bilaplacian}
  \vspace{-0.5ex}
  \caption{\textbf{Illustration of \cref{eq:ttc-general} for the biharmonic operator}, \ie the five values of $\vj$ (integer points that fulfill $\lVert \vj \rVert_1 = 4$) and their coefficients $\gamma_{\vi, \vj}$ for interpolating the desired mixed partials.}
...
\Cref{fig:ttc_biharm_coeffs} illustrates the five multi-indices $\vj$ characterizing the $4$-jets we need to interpolate $\langle \partial^4 \vf(\vx_0), \ve_{d_1}^{\otimes 2} \otimes \ve_{d_2}^{\otimes 2} \rangle$, and the coefficients $\gamma_{\vi, \vj}$.
Their definition \cref{eq:ttc_coeff} shows the equality of $\gamma_{\vi, \vj}$ for $\vj = (4,0)$ and $\vj = (0, 4)$, as well as $\vj = (3, 1)$ and $\vj = (1, 3)$.
...
\paragraph{Summary and relation to other approaches for computing mixed partials.} Overall, this demonstrates the relevance of collapsing standard Taylor mode. It is applicable to computing \emph{general} linear differential operators beyond Laplacians.
While our method seems daunting, it uses $K$-jets to compute arbitrary mixed partials of order $K$.
There are more `pedagogical' approaches, which however require evaluating $(K'>K)$-jets and this becomes expensive, see \cref{sec:appendix_ttc_other_methods} where we present a pedagogical (but inefficient) approach to compute the biharmonic with $6$-jets similar to the approach proposed in \cite[][\S{}F]{shi2024stochastic}.
