...
\subsection{Collapsed Taylor-mode for arbitrary mixed-derivatives}
...
\cref{sec:appendix_ttc} provides supplementary details for the multi-index notation and the coefficients in the formula. From this reconstruction we conclude that \cref{eq:sums-k-directional} can be rewritten to
...
As the inventors of the formula already recognized, the coefficients  $\gamma_{\vp\vq} \frac{1}{K!}$ only depend on the problem structure (i.e., on $K$, $P$ and $\vp$) and are independent of the function $\vf$ and the directions $\vv_{n_k}$. Therefore, we can pull out the inner sum as well as the coefficients and end with the expression
\begin{equation}
    \sum_{\underset{\vq \in \mathbb{N}^P}{|\vq| = K}}
    \gamma_{\vp \vq}
    \frac{1}{K!}
...
\paragraph{Biharmonic Operator} A prominent differential operator with mixed-partials is represented by the Biharmonic Operator. \todo{cite} Below, we derive the formula to compute the operator using collapsed Taylor-mode.
Our approach first applies \cref{eq:ttc_general} to the Biharmonic Operator \cref{eq:biharm}. The parameters are selected to $K = 4, P = 2, N_1 = N_2 = D, \vp=(2, 2)$ and the directions to the unit vectors $\vv_{n_1} = \ve_{n_1}$ and $\vv_{n_2} = \ve_{n_2}$. The result is shown in the appendix \cref{eq:ttc_for_biharm} and the coefficients are listed in \cref{tab:ttc_biharm_coeffs}.
...
The last term can be further simplified by leveraging the symmetry of the direction $2 \ve_i + 2 \ve_j$. This simplification leads to the following final efficient jet-based formula for the Biharmonic Operator :
\begin{equation} \label{eq:ttc_for_biharm_final}
    ...
    \end{aligned}
\end{equation}
The final equation is based on $D + D(D-1) + \frac{D(D-1)}{2}$ different $4$-jets, where we can collapse the highest coefficients. From this we conclude that our formula for the Biharmonic Operator has the effort of propagating $1 + 3D + 1 + 1 + 3 \cdot D(D-1) + 1 + 1 + 3 \frac{D(D-1)}{2} + 1 = 9\frac{D^2}{2} - 3\frac{D}{2} + 6$ vectors through every node in the computational graph, when computed with our proposed collapsed Taylor-mode. As emphasized before, all jets are independent and can be computed in parallel, which provides further benefits for the run-time complexity. 

The above procedure describes a generic approach for computing linear differential operators with Taylor-Mode AD that can be combined with our proposed collapsing method. We demonstrated it on the Biharmonic Operator, as it is a commonly used operator of degree greater than two. We also considered other approach, which underperform our contribution. For a comparison see \cref{sec:appendix_ttc_other_methods}. It has to be remarked, because of its special structure, there is an other efficient scheme for computing the Biharmonic Operator that nests twice the Laplacian. In the appendix, we briefly discuss this approach in combination with our collapsed Taylor-Mode. However, our focus here was to show the reader that our idea applies to other differential operators, therefore making it a relevant contribution.
