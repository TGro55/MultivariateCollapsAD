...
Our approach first applies \cref{eq:ttc_general} to the Biharmonic Operator \cref{eq:biharm} with the parameters selected as stated aboveto $K = 4, P = 2, N_1 = N_2 = D, \vp=(2, 2)$ and the directions to the unit vectors $\vv_{n_1} = \ve_{n_1}$ and $\vv_{n_2} = \ve_{n_2}$. The result is shown in the appendix \cref{eq:ttc_for_biharm} and the coefficients are listed in \cref{tab:ttc_biharm_coeffs}.
