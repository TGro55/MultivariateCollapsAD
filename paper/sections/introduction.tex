Using neural networks to learn functions constrained by physical laws is a popular trend in scientific machine learning \cite{carleo2017solving, pfau2020ab, hermann2020deep, karniadakis2021physics, raissi2019physics, hu2023hutchinson, sun2020global}.
...
VMC employs neural networks as ansatz for the Schr\"odinger equation \cite{carleo2017solving, pfau2020ab, hermann2020deep} and demands computing the net's Laplacian, \ie, the Hessian trace.
PINNs represent PDE solutions as a neural net and train it by minimizing the residuals of the governing equations \cite{raissi2019physics, karniadakis2021physics}. For instance, Kolmogorov-type equations, like the Fokker-Planck and Black-Scholes equation, require weighted second-order derivatives on high-dimensional spatial domains \cite{hu2023hutchinson, sun2024dynamical}.
PINN applications for elasticity problems use the biharmonic operator \cite{vahab_physics-informed_2022, hu2023hutchinson, vikas_biharm, shi2024stochastic}, which contains fourth-order derivatives.
...
A promising alternative is \emph{Taylor-mode AD}~\cite[or simply \emph{Taylor-mode},][\S13]{griewank2008evaluating}, introduced to the ML community in \citeyear{bettencourt2019taylor}, which scales polynomially \wrt degree in time and memory \cite{griewank_evaluating_1999}.
However, we observe empirically that vanilla Taylor mode may not be enough to beat nesting: when evaluating the Laplacian of a 5-layer MLP, using JAX's \emph{Taylor mode is 50\% slower than nested backpropagation} that computes, then traces, the Hessian via Hessian-vector products \cite{pearlmutter1994fast,dagreou2024how}.
...
For the Laplacian, \citet{li2023forward, li2024dof} developed a special forward propagation framework called the \emph{forward Laplacian}, whose JAX implementation \cite{gao2023folx} is \emph{roughly twice as fast as nested first-order AD}.
...
Concurrently, \citet{shi2024stochastic} derived highly-efficient stochastic approximations of differential operators in high dimensions by evaluating Taylor mode along suitably sampled random directions.
...
Based on this linearity, we identify an optimization mechanism to rewrite the computational graph of standard Taylor mode that is applicable to general linear PDE operators and randomized Taylor mode:
...
\item \textbf{We propose optimizing standard Taylor mode by collapsing the highest Taylor coefficients,} directly propagating their sum, rather than propagating then summing (\cref{fig:visual-abstract}).
...
We implement a Taylor mode library for PyTorch \cite{paszke2019pytorch} that realizes the graph simplifications with \texttt{torch.fx} \cite{reed2022torch}.
On popular PDE operators, we empirically find that collapsed Taylor mode achieves the theoretically expected performance improvements of standard Taylor mode, and consistently outperforms nested first-order AD.
\end{enumerate}
