...
\begin{align*}%\label{eq:sum-taylor-mode-efficient}
  %\begin{split} 
        ...
        &\overset{\text{slice}}{\to} \textcolor{\colorcTM}{\sum_{r=1}^R \vg_{K,r}}
          \overset{\{\vx_{1,r} = \vv_r, \vx_{2,r} = \vzero, \dots, \vx_{K, r} = \vzero \}}{=} 
          \sum_{r=1}^R \left<
          \partial^K \vf(\vx_0), 
          \otimes_{k=1}^K \vv_r
          \right>
...
\paragraph{Vanilla Taylor Mode AD}
If we have $f = \vg \circ \vh$ and apply the vanilla Taylor mode (see \cref{eq:sum-taylor-mode-naive}) to compute the Laplacian we obtain
{
