...
from jet.exp.exp_multijet.execute import HERE as SCRIPT
from jet.exp.exp_multijet.execute import RAWDIR
from jet.exp.exp_multijet.execute import savepath as savepath_raw
from jet.exp.utils import run_verbose, to_string

# Different Strategies for different Experiments
SUPPORTED_STRATEGIES_MULTIJET = [
    "hessian_trace",
    "jet_naive",
    "jet_simplified",
    "multijet_naive",
...
SUPPORTED_STRATEGIES_MULTIJET_SYM = [
    "hessian_trace",
    "jet_naive",
    "jet_simplified",
    "multijet_sym_naive",
...
EXPERIMENTS = [
    ...
    # Experiment 2: Exact Bi-Laplacian with multijet, vary batch size
    (  # Experiment name, must be unique
        "bilaplacian_with_multijet_vary_batch_size",
        # Experiment parameters
        {
            "architectures": ["tanh_mlp_768_768_512_512_1"],
            "dims": [5],
            "batch_sizes": linspace(1, 400, 10).int().unique().tolist(),
            "strategies": SUPPORTED_STRATEGIES_MULTIJET,
            "devices": ["cuda"],
    ...
    ### Added for multijet
    # Experiment 3: Exact Bi-Laplacian with multijet, vary batch size
    (  # Experiment name, must be unique
        "bilaplacian_with_sym_multijet_vary_batch_size",
        # Experiment parameters
        {
            "architectures": ["tanh_mlp_768_768_512_512_1"],
            "dims": [5],
            "batch_sizes": linspace(1, 512, 10).int().unique().tolist(),
            "strategies": SUPPORTED_STRATEGIES_MULTIJET_SYM,
            "devices": ["cuda"],
            "operator": "bilaplacian",
            "compiled": [False],
        },
